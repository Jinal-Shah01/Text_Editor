...
class Text_Editor:
    ...
    def copyFile(self,event):
        ...
        self.area = self.textArea.get(1.0,END + "-1c")

    def pasteFile(self,event):
        self.textArea.insert(INSERT,self.area)
    ...
    def findString(self):
        self.find = simpleDialog.askstring("Find","Enter text")
        ...
        for y in range(0,len(text_Array)):
            if type(text_Array[y]) == list:
                for z in range(0,len(text_Array[y])):
                    if text_Array[y][z] == self.find:
                        text_Array[y][z] = replace
            else:
                if text_Array[y] == self.find:
